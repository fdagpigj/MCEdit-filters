			...
			if (x, y, z) in box:
				if e["id"].value == "Sign":
					...
					try:
						...
						print(e["Text2"].value)
						try:
							name = (json.loads(e["Text1"].value)["text"].replace(u"\uf701","").encode('utf-8')
							 + json.loads(e["Text2"].value)["text"].replace(u"\uf701","").encode('utf-8'))
							...
							name = (json.loads(e["Text1"].value[1:-1])["text"].replace(u"\uf701","").encode('utf-8')
							 + json.loads(e["Text2"].value[1:-1])["text"].replace(u"\uf701","").encode('utf-8'))
							command = "/summon ArmorStand %s %s %s {CustomName:\"%s\",%s}" % (x,y,z, name, tags)
							print(command)
							a = 1
						if a == 0:
